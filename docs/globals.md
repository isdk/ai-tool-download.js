[**@isdk/ai-tool-downloader**](README.md) • **Docs**

***

# @isdk/ai-tool-downloader

## Classes

- [ChunkDownload](classes/ChunkDownload.md)
- [DownloadFunc](classes/DownloadFunc.md)
- [FileDownload](classes/FileDownload.md)

## Interfaces

- [ChunkOptions](interfaces/ChunkOptions.md)
- [DownloadConfiguration](interfaces/DownloadConfiguration.md)
- [FileDownloadOptions](interfaces/FileDownloadOptions.md)
- [UrlMetaInfo](interfaces/UrlMetaInfo.md)

## Type Aliases

- [FileDownloadStatus](type-aliases/FileDownloadStatus.md)

## Variables

- [AlreadyDownloadErrCode](variables/AlreadyDownloadErrCode.md)
- [defaultChunkSizeInBytes](variables/defaultChunkSizeInBytes.md)
- [defaultConcurrency](variables/defaultConcurrency.md)
- [download](variables/download.md)
- [DownloadErrorEventName](variables/DownloadErrorEventName.md)
- [DownloadName](variables/DownloadName.md)
- [DownloadProgressEventName](variables/DownloadProgressEventName.md)
- [DownloadStatusEventName](variables/DownloadStatusEventName.md)
- [minSplitSizeInBytes](variables/minSplitSizeInBytes.md)
- [RangeRequestErrCode](variables/RangeRequestErrCode.md)

## Functions

- [concatStreamTo](functions/concatStreamTo.md)
- [createWritableStream](functions/createWritableStream.md)
- [getFilenameFromResponse](functions/getFilenameFromResponse.md)
- [getFilenameFromUrl](functions/getFilenameFromUrl.md)
- [getUrlMetaInfo](functions/getUrlMetaInfo.md)
